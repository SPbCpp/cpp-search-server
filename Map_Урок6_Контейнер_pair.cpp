//Map
//
//Урок 6. Контейнер pair
//
//Задание
//
//Не всегда ответ на запрос соответствует ожиданиям.
//Чем ближе результат к ожидаемому, тем выше его релевантность
//исходному запросу. Научите свою поисковую систему считать
//релевантность возвращённого в ответе документа — то есть количество
//уникальных слов, пересекающихся с запросом.
//Эту релевантность нужно возвращать вместе с идентификатором документа
//как пару из двух целых чисел{ document_id, relevance }. Иными словами,
...
//         funny fat 	a fat cat sat on a matand ate a 	     
//                                                                2
//           cat                     fat rat
	...
	const string& query)
{
	...
	for (const string& word : query_words)
	{
		...
		for (const int document_id : word_to_documents.at(word))
		{
			...
			document_to_relevance.insert(pair{ document_id, relevance });
			relevance = 0;
		}
		relevance = 0;
	}
